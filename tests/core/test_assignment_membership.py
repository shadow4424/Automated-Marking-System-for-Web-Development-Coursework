...
import ams.core.database as _database
from ams.core.user_store import create_user
...
def test_list_assignments_includes_additional_teachers(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(_database, "_DEFAULT_DB_PATH", db_path)

    _database.init_db()
    create_user("teacher_owner", "Owner", "Teacher", "owner@example.com", "Pass123", role="teacher")
    create_user("teacher_peer", "Peer", "Teacher", "peer@example.com", "Pass123", role="teacher")

    assert create_assignment(
    ...
    owner_assignments = list_assignments(teacher_id="teacher_owner")
    peer_assignments = list_assignments(teacher_id="teacher_peer")
