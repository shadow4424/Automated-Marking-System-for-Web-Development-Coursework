...
def run_batch(
    ...
    items = discover_batch_items(submissions_dir)
    records: List[dict] = []
    ...
    for item in items:
        ...
        try:
            ...
            report_path = pipeline.run(submission_path=item.path, workspace_path=workspace_path, profile=profile)
...
def aggregate_batch(records: List[dict], finding_counts: Counter[str], failure_reason_counts: Counter[str], profile: str) -> dict:
    ...
    # ensure keys present for omitted components
